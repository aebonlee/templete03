...
## 학습 콘텐츠 형태 (대면 중심 + 웹 자료)
이 사이트는 **대면 강의 + 웹 학습자료**가 기본이고 영상은 선택 보조 수단이다.
레슨(`site.js`)은 `kind: 'reading' | 'video'`, 읽기 자료 `content`(블록 배열),
선택 `videoId`(유튜브)를 가진다. 블록 렌더러는 `components/LessonContent.jsx`
(h/p/callout/list/steps/code/image/quote/divider). `callout`은 형광펜 시그니처를
학습자료로 확장한 강조 박스. 영상+자료 동시 사용 가능.

## 디렉터리
- `src/config/site.js` — 콘텐츠(브랜드/히어로/모듈/가격/레슨 자료). **프로젝트마다 교체**
